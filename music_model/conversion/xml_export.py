...
def _parse_to_xml(score: Score, pretty: bool = False) -> str:
    ...
    created_octave_shifts = set()   # keep track to do these only once
    ...
    def create_title(xml_root):
        ...
        work_title.text = "Songtitle"  # TODO: save title in score
    ...
    def create_attributes_if_necessary(xml_measure, part, onset):
        ...
        xml_attributes = None
        # set attributes - oder matters for valid xml!
        if is_first_attributes:
            xml_attributes = ET.SubElement(xml_measure, "attributes")
            ET.SubElement(xml_attributes, "divisions").text = str(division)
        # key and time only set once for both staffs? - use treble staff for now TODO investigate
        staff = part._staffs[0]
        if staff._key_signatures.get(onset):     # only True (not None) if key exists
            if xml_attributes is None:
                xml_attributes = ET.SubElement(xml_measure, "attributes")
            create_key(xml_attributes, staff._key_signatures.get(onset))
        if staff._time_signatures.get(onset):     # only True (not None) if key exists
            if xml_attributes is None:
                xml_attributes = ET.SubElement(xml_measure, "attributes")
            create_time(xml_attributes, staff._time_signatures.get(onset))
        if is_first_attributes:
            ET.SubElement(xml_attributes, "staves").text = str(len(part._staffs))
        for staff in part._staffs.values():
            if staff._clefs.get(onset):     # only True (not None) if key exists
                if xml_attributes is None:
                    xml_attributes = ET.SubElement(xml_measure, "attributes")
                create_clef(xml_attributes, staff._clefs.get(onset), staff._id + 1)
        is_first_attributes = False
    ...
    def create_tuplet_if_necessary(xml_note, chord_rest):
        ...
        for i in range(len(tuplets)):
            ...
            if tuplet.get_first_chord_or_rest() is chord_rest:
                ...
                # TODO get bracket from if beam exists with same elements
    ...
    def create_measure(xml_measure, part, measure):
        ...
        for voice in part._voices.values():
            ...
            for chord_rest in voice.get_chords_and_rests(measure._onset, measure.get_offset(), (True, False)):
                ...
                # check if attributes are required at current onset
                create_attributes_if_necessary(xml_measure, part, onset)
...
def show(score: Score, dpi: int = 100, margin_in_px: int = 0):
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        display(Image(filename=temp + "\score" + "-1" + ".png"))  # TODO handle multiple sites and -01
